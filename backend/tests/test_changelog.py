...
# 예전엔 items[0] 이 2.3.10 이라 맨 위 항목을 봤다. 새 버전이 쌓이면서
# 맨 위가 바뀌었고, 새 항목에 '블룸' 이 들어가자 엉뚱하게 깨졌다.
# 2.3.10 을 이름으로 찾아서 본다.
_v2310 = [e for e in items if e['version'] == '2.3.10']
assert _v2310, '2.3.10 항목이 없다'
assert '블룸' not in _v2310[0]['body'], '2.3.10 본문에 옛 노트가 섞였다'
ok += 1
